...
| **Gemma 4 12B** | **SKILL.state, same, effort low (budget-exhaustion retry in place)** | **1.00** (200/200) | 0.69M | flat 8.4K chars, 13 s/call | 47 min | 8 thinking-budget exhaustions absorbed by retry-with-2× budget |
| Gemma 4 12B | SKILL.state, same, effort low (before the retry) | 1.00 over 76 | 0.25M | flat | then stalled | three consecutive budget exhaustions were counted as decision failures |
...
- **Small model: the interface and the reasoning budget, not the memory, were the problems.** With the ledger correct in
  its state, Gemma still answered queries with the query's *name* until the tool's argument description carried example
  values — then 0.54 → 0.91 at zero reasoning effort, and **1.00 with a little thinking** once budget exhaustion was handled
  as a budget event instead of a decision failure. Every earlier "Gemma can't" number in these docs was measured with
  reasoning switched off. As a ReAct agent it scored 0.99 with

...
def configure_logging(level: str = "INFO", *, json_output: bool = False, stream: Any = None) -> None:
    root = logging.getLogger("mnestic")
...
def get_logger(name: str, **bound: Any) -> FieldsAdapter:
    return FieldsAdapter(logging.getLogger(f"mnestic.{name}"), bound)
...
def maybe_configure_logfire(enabled: bool) -> bool:
    ...
    try:
        ...
        logging.getLogger("mnestic").warning("MNESTIC_LOGFIRE set but logfire is not installed (uv add logfire)")
        return False
    logfire.configure()

"""``skillstate`` CLI — inspect exactly what the agent believes and exactly what the model sees."""
...
from skillstate.agent.reasoner import Reasoner, ScriptedReasoner
...
from skillstate.observability.logging import configure_logging, maybe_configure_logfire
...
def _reasoner(cfg: RuntimeConfig, skill: SkillSpecification | None, model: str | None) -> Reasoner:
    ...
    if name == "mock":
        ...
        from skillstate.benchmarks.scripts import load_script
...
def cmd_graph(cfg: RuntimeConfig, args: argparse.Namespace) -> int:
    from skillstate.graph.runtime import build_graph
...
def cmd_doctor(cfg: RuntimeConfig, args: argparse.Namespace) -> int:
    ...
    ok = True
    print(f"skillstate {__version__}")
    ...
    if cfg.model != "mock":
        try:
            from skillstate.agent.pydantic_ai_reasoner import build_model
...
def build_parser() -> argparse.ArgumentParser:
    ...
    common.add_argument("--db", type=Path, default=argparse.SUPPRESS, help="SQLite path (default .skillstate/skillstate.db or SKILLSTATE_DB_PATH)")
    common.add_argument("--workspace", type=Path, default=argparse.SUPPRESS, help="workspace root (default cwd or SKILLSTATE_WORKSPACE)")
    ...
    p = argparse.ArgumentParser(prog="skillstate", description="SKILL.state agent runtime", parents=[common])

"""Runtime configuration. Environment variables use the ``SKILLSTATE_`` prefix."""
...
class RuntimeConfig(BaseModel):
    ...
    db_path: Path = Field(default=Path(".skillstate/skillstate.db"))
    ...
    def from_env(cls, **overrides: Any) -> RuntimeConfig:
        env = os.environ
        values: dict[str, Any] = {}
        if v := env.get("SKILLSTATE_DB_PATH"):
            values["db_path"] = Path(v)
        if v := env.get("SKILLSTATE_WORKSPACE"):
            values["workspace_root"] = Path(v)
        if v := env.get("SKILLSTATE_SKILLS_DIRS"):
            values["skills_dirs"] = [Path(p) for p in v.split(os.pathsep) if p]
        if v := env.get("SKILLSTATE_MODEL"):
            values["model"] = v
        if v := env.get("SKILLSTATE_OUTPUT_MODE"):
            values["output_mode"] = v
        if v := env.get("SKILLSTATE_MODEL_RETRIES"):
            values["model_retries"] = int(v)
        if v := env.get("SKILLSTATE_SHELL_MODE"):
            values["shell"] = ShellPolicy(mode=v)  # type: ignore[arg-type]
        if v := env.get("SKILLSTATE_LOG_LEVEL"):
            values["log_level"] = v
        if env.get("SKILLSTATE_LOG_JSON") in {"1", "true", "yes"}:
            values["log_json"] = True
        if env.get("SKILLSTATE_LOGFIRE") in {"1", "true", "yes"}:
            values["logfire"] = True

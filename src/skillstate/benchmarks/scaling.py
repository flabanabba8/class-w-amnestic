...
from skillstate.tools.base import Tool, ToolContext, ToolRegistry, ToolResult
...
async def _run(steps: int, observation_chars: int, checkpoints: list[int] | None, db_path: Path) -> dict[str, Any]:
    ...
    rows = [{"step": c, "skillstate_chars": sizes[c - 1], "skillstate_tokens_est": tokens[c - 1], "state_section_bytes": state_bytes[c - 1],
    ...
    return {
        ...
        "skillstate": {"first": sizes[0], "min": min(sizes), "max": max(sizes), "mean": round(statistics.mean(sizes), 1),
...
def _skill_prompt_chars() -> str:
    from skillstate.context.builder import OUTPUT_CONTRACT, ContextBuilder
...
def render_report(r: dict[str, Any]) -> str:
    ...
    for row in r["rows"]:
        lines.append(f"| {row['step']} | {row['skillstate_chars']:,} | {row['skillstate_tokens_est']:,} | {row['state_section_bytes']:,} | {row['react_chars']:,} |")
    s, b = r["skillstate"], r["react"]
    lines += [
        "", "## Summary", "",
